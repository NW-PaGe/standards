...
4. Now in an R script you can call that password _and_ hide the credentials instead of writting the password in the script for everyone to see:
...
**This is a _template_ file, so it will not have any passwords/secrets in it**. Its only purpose is to provide an example copy of what a user's `creds.yml` file needs to look like.
...
:::
    ...
    - add the path like this `echo PATH=$PATH:/user/local/bin/git-secrets`
    - hit `CTRL + O` then `ENTER` to save
    - hit `CTRL + X` to exit
    - start a new terminal and write this to see your path variables. `git-secrets` should be in there somewhere now
